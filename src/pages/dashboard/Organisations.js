...
import { addDepartment, getAllDepartments } from "../../api/services/departments";
...
export default function Organisations() {
  ...
  const fetchAllAdmins = async () => {
    const response = await getUsersForRole({ role: "org-admin" });
  ...
  const handleUpdate = async () => {
    const response = await updateOrganisation(selectedOrganisationId, {
      name: organisationName,
      admin: adminId,
      departments: departmentIds,
    });
  ...
  const handleDelete = async () => {
    const response = await deleteOrganisation(selectedOrganisationId);
    if (response.status === "success") {
      handleSuccessResponse();
    }
  };
  ...
  return (
    <>
            ...
            {organisations &&
              organisations.map((row, index) => (
                <TableRow key={row._id} sx={{ "&:last-child td, &:last-child th": { border: 0 }, padding: 8 }}>
                  ...
                  <TableCell align="left">{row?.departments?.map((d) => d.name).join(", ")}</TableCell>
                  <TableCell align="left">
                    <IconButton
                      onClick={() => {
                        setShowEditModal(true);
                        setOrganisationName(row.name);
                        setSelectedOrganisationId(row._id);
                        setAdminId(row.admin?._id);
                        setDepartmentIds(row.departments?.map((d) => d._id));
                      }}>
      ...
      <Dialog open={showAddModal} onClose={closeDialog} fullWidth={true} maxWidth="xs">
        <DialogTitle style={{ paddingBottom: 0 }}>Add Organisation</DialogTitle>
          ...
          <FormControl sx={{ mt: 3, width: "100%" }}>
            ...
            <Select
              ...
              onChange={({ target: { value } }) =>
                setDepartmentIds(typeof value === "string" ? value.split(",") : value)
              }
              ...
              {departments &&
                departments.map((d) => (
                  <MenuItem key={d._id} value={d._id}>
                    <Checkbox checked={departmentIds.includes(d._id)} />
        ...
        <DialogActions>
          ...
          <Button onClick={handleAdd} disabled={!organisationName || !adminId || departmentIds.length === 0}>
            Add
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={showEditModal} onClose={closeDialog} fullWidth={true} maxWidth="xs">
        <DialogTitle style={{ paddingBottom: 0 }}>Edit Organisation</DialogTitle>
        <DialogContentText></DialogContentText>
        <DialogContent>
          <TextField
            autoFocus
        ...
        <DialogActions>
          <Button onClick={closeDialog}>Cancel</Button>
          <Button onClick={handleUpdate} disabled={!organisationName || !adminId || departmentIds.length === 0}>
            Save
          </Button>
        </DialogActions>
      </Dialog>
